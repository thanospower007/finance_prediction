document.addEventListener('DOMContentLoaded', () => {
    ...
    window.switchMainMode = function (mode) {
        ...
        // Asegurar que los paneles estén visibles cuando se regrese al análisis
        if (mode === 'analysis') {
    ...
    async function renderSimulationResult(data, isError = false) {
        ...
        // Actualizar los paneles con la información del ticker simulado
        // Obtener información básica del ticker para mostrar en los paneles
        try {
            const ticker = document.getElementById('simTicker').value.toUpperCase();
            const modelId = document.getElementById('aiModelSelect').value;

            // Hacer una llamada adicional para obtener la información del ticker
            // Usar la fecha de inicio de la simulación si existe, de lo contrario obtener datos actuales
            let analysisUrl = `/api/analyze/${ticker}?model=${modelId}`;
            const startDate = document.getElementById('simStartDate').value;
            if (startDate) {
                analysisUrl += `&date=${startDate}`;
            }
            ...
            if (response.ok) {
                ...
                if (analysisData.main) {
                    // Mostrar el panel A con la información del ticker simulado
                    document.getElementById('panel-a').classList.remove('hidden');

                    // Actualizar el badge de simulación
                    document.getElementById('simulation-badge-a').classList.remove('hidden');

                    // Actualizar el panel con la información obtenida
                    populatePanel('a', analysisData.main);

                    // Mostrar el gráfico de precios
                    renderChart(analysisData.main, null);

                    // Renderizar la importancia de características si está disponible
                    if (analysisData.main.ai_analysis) {
                        renderFeatureImportance(analysisData.main.ai_analysis, 'a');
                    }
                }
            }
        } catch (err) {
            console.error('Error al actualizar los paneles con la información del ticker simulado:', err);
        }
    }
